...
// in project-state file we're not just exporting a class or interface, but also exporting a constant 'projectState'

// we are importing this constant in multiple other files (project-list.ts, project-input.ts)

// how often does this constant run?
// once per import statement or once for the entire app?

// It turns out it runs once when the file is imported for the first time by any other file

// if another file then imports that same file again, it doesn't run again

// something you should know so that you can plan you app appropriately